...
def repeat_vs_avg(exp):
    ...
    # Aggregate
    trace_repeat = go.Bar(x=y_repeat, y=x_param, marker=dict(color="lightgrey", line=dict(color="#160D0D", width=1)), name="repeat", orientation="h")
    trace_avg = go.Scatter(
        ...
        name="avarage",
        error_x=dict(type="data", color="rgba(0,176,246,0.3)", array=y_deviation, visible=True),
    )
