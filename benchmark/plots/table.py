...
def table(experiments):
    ...
    trace = go.Table(
        header=dict(values=header, font=dict(color="#2e2f30", size=12), fill=dict(color=h_colors), align=["left"] * 5, height=40),
        cells=dict(values=cells, line=dict(color="#7D7F80"), fill=dict(color="#EDFAFF"), align=["left"] * 5),
    )
