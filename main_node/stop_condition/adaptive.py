...
class AdaptiveType(StopCondition):
    ...
    def __init__(self, stop_condition_parameters: dict, experiment_description: dict, experiment_id: str):
        ...
        if math.isfinite(search_space_size):
            self.max_configs = round(stop_condition_parameters["Parameters"]["SearchSpacePercentage"] / 100 * float(search_space_size))
            if os.environ.get("TEST_MODE") != "UNIT_TEST":
                self.start_threads()
        else:
            temp_msg = (
                "Unable to use Adaptive Stop Condition when size of Search Space is infinite. "
