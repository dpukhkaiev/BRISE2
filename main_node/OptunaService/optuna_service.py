...
SEARCH_SPACE_PREFIX = "Context.SearchSpace"  # matches core_entities/search_space.py
...
def normalize_search_space(search_space):
    return {
        f"{SEARCH_SPACE_PREFIX}.{name}": data
        for name, data in search_space.items()
        if name != "Structure"
    }
...
def get_axis_values(
    ...
    parameter_search_space = search_space.get(parameter)
    ...
    parameter_type = parameter_search_space.get("Type")

    # ORD / CAT
    if (
        parameter_type == "OrdinalHyperparameter"
        or isinstance(parameter_search_space.get("Categories"), list)
    ):
        categories = parameter_search_space["Categories"]
