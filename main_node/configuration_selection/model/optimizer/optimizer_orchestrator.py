...
class OptimizerOrchestrator:
    def get_optimizer(self, optimizer_description: Dict, region: Tuple, objectives: Dict) -> Optimizer:
        ...
        assert len(keys) == 1
        feature_name = keys[0]
        optimizer_class = reflective_class_import(class_name=optimizer_description["Instance"][feature_name]["Type"], folder_path="configuration_selection/model/optimizer")
