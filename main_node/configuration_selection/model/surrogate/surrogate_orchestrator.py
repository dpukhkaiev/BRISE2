...
class SurrogateOrchestrator:
    ...
    def get_surrogate(self, surrogate_description: Dict, region: Tuple, objectives: Dict) -> Surrogate:
        ...
        assert len(keys) == 1
        feature_name = keys[0]
        surrogate_class = reflective_class_import(class_name=surrogate_description["Instance"][feature_name]["Type"], folder_path="configuration_selection/model/surrogate")
