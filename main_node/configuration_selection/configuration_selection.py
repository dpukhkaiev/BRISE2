...
class ConfigurationSelection:
    ...
    def __init__(self, experiment: Experiment):
        ...
        self.predictor: Predictor = Predictor(self.experiment.unique_id, self.experiment.description, self.experiment.search_space)
        # check if TL is available
        if "TransferLearning" in self.experiment.description.keys():
            self.transfer_is_enabled = True
            self.transfer_learning_orchestrator = TransferLearningOrchestrator(self.experiment.description, self.experiment.unique_id)
        else:
    ...
    def send_new_configurations_to_measure(self, ch, method, properties, body) -> Tuple[List[Configuration], List[Configuration]]:
        """
        ...
        number_of_predicted_configs = min([model.candidate_selector.number_of_points for model in self.predictor.mapping_region_model.values()])
        ...
        if not self.transfer_is_enabled:
            ...
            if similar_experiments is None:
                ...
                if model_transfer_module is not None:
                    transferred_mapping_region_model = self.transfer_learning_orchestrator.transfer_submodules["Model_transfer"].recommend_best_model(similar_experiments)
                ...
                configuration_transfer_module = self.transfer_learning_orchestrator.transfer_submodules["Configuration_transfer"]
                if configuration_transfer_module is not None:
                    transferred_configurations = self.transfer_learning_orchestrator.transfer_submodules["Configuration_transfer"].transfer_configurations(similar_experiments)
                    transferred_configurations = list(filter(lambda tc: tc.parameters not in [mc.parameters for mc in self.experiment.measured_configurations], transferred_configurations))
                    self.logger.info(f"Identified a set of promising configurations from a similar experiment, " f"{transferred_configurations}")
                    ...
                    if configuration_transfer_module.is_few_shot:
                        ...
                        self.logger.info(f"Measuring the best configuration from the former experiment, " f"if it has not been measured yet: " f"{transferred_configurations[0]}")
        ...
        for c in predicted_configs:
            if c not in self.experiment.evaluated_configurations:
                ...
                if os.environ.get("TEST_MODE") != "UNIT_TEST":
                    publish(exchange="stop_experiment_exchange", routing_key=self.experiment.unique_id, body=msg)
        ...
        for c in configs_to_be_evaluated:
            ...
            if self.experiment.search_space.is_flat:
                c_to_send.parameters = self.experiment.search_space.transform_flat_parameters_to_hierarchic(c.parameters)
            hierarchical_configs.append(c_to_send)
            self.sub.send("log", "info", message=temp_msg)
            if os.environ.get("TEST_MODE") != "UNIT_TEST":
                publish(exchange="measure_new_configuration_exchange", routing_key=self.experiment.unique_id, body=json.dumps({"configuration": c_to_send.to_json()}))
    ...
    class _EventServiceConnection(RabbitMQConnection):
        ...
        def bind_and_consume(self):
            ...
            self.channel.basic_consume(queue="get_new_configuration_exchange" + self.experiment_id, auto_ack=True, on_message_callback=self.configuration_selection.send_new_configurations_to_measure)
