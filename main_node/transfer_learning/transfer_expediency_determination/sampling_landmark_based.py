...
class SamplingLandmarkBased(TransferExpediencyAnalyser):
    def __init__(self, ted_description: dict, experiment_id: str):
        ...
        if quantity_type == "AdaptiveQuantity":
            ...
            self.number_of_similar_experiments = (self.ted_description)[self.feature_name]["ExperimentsQuantity"]["FixedQuantity"]["NumberOfSimilarExperiments"]
    ...
    def _get_comparator(self):
        ...
        comparator_class = reflective_class_import(class_name=comparator_type, folder_path="transfer_learning/transfer_expediency_determination")
    ...
    def _get_clustering_algorithm(self):
        ...
        """
        clustering_key = (list(self.ted_description[self.feature_name]["ExperimentsQuantity"]["AdaptiveQuantity"]["Clustering"].keys()))[0]
        clustering_type = (self.ted_description)[self.feature_name]["ExperimentsQuantity"]["AdaptiveQuantity"]["Clustering"][clustering_key]["Type"]
        clustering_algorithm_class = reflective_class_import(class_name=clustering_type, folder_path="transfer_learning/transfer_expediency_determination/clustering")
        return clustering_algorithm_class(self.ted_description[self.feature_name]["ExperimentsQuantity"]["AdaptiveQuantity"]["Clustering"][clustering_key])
    ...
    def analyse_experiments_similarity(self) -> Union[List, None]:
        ...
        number_of_measured_configurations = self.database.get_last_record_by_experiment_id("Experiment_state", self.experiment_id)["Number_of_measured_configs"]
        ...
        if len(sorted_similar_experiments) > 0:
            if self.clustering_algorithm is not None:
                similar_experiments = self.clustering_algorithm.cluster_top_most_similar_experiments(sorted_similar_experiments)
            else:
                similar_experiments = [sorted_similar_experiments[: self.number_of_similar_experiments]]
