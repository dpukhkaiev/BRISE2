...
class TransferLearningOrchestrator:
    ...
    def __init__(self, experiment_description: dict, experiment_id: str):
        ...
        self.ted_module = ted_orchestrator.get_ted_module(self.experiment_description["TransferLearning"]["TransferExpediencyDetermination"], self.experiment_id)
        ...
        if "MultiTaskLearning" in self.experiment_description["TransferLearning"].keys():
            ...
            self.transfer_submodules["Configuration_transfer"] = self.mtl_orchestrator.get_mtl_module(self.experiment_description, self.experiment_id)
        if "ModelRecommendation" in self.experiment_description["TransferLearning"].keys():
            ...
            self.transfer_submodules["Model_transfer"] = self.mr_orchestrator.get_mr_module(self.experiment_description, self.experiment_id)
