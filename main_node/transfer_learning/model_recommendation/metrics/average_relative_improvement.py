...
class AverageRelativeImprovementMetric(ModelPerformanceMetric):
    ...
    def compute(self, improvement_curve: List, prediction_infos: List, start_index: int, end_index: int, multi_model: bool) -> Dict:
        ...
        for i, prediction_info in enumerate(prediction_infos[start_index:end_index]):
            ...
            if len(combinations) == 0 or any([combinations[-1][r]["Model"] != p_i["Model"] for r, p_i in prediction_info.items()]):
                    ...
                    for j, _ in enumerate(start_indices_for_combinations):
                        if j + 1 < len(start_indices_for_combinations):
                            if self.is_minimization_experiment:
                                relative_improvement = (abs(improvement_curve[start_indices_for_combinations[j]]) - abs(improvement_curve[start_indices_for_combinations[j + 1]])) / abs(
                                    improvement_curve[start_indices_for_combinations[j]]
                                )
                            else:
                                relative_improvement = (abs(improvement_curve[start_indices_for_combinations[j + 1]]) - abs(improvement_curve[start_indices_for_combinations[j]])) / abs(
                                    improvement_curve[start_indices_for_combinations[j + 1]]
                                )
                            improvements.append(
                                {
                                    "Model_combination": combinations[j],
                                    "average_relative_improvement": relative_improvement / (start_indices_for_combinations[j + 1] - start_indices_for_combinations[j]),
                                }
                            )
                        # for the case when the same model combination was used on the whole interval
                        else:
                            if self.is_minimization_experiment:
                                relative_improvement = (abs(improvement_curve[start_indices_for_combinations[j]]) - abs(improvement_curve[end_index])) / abs(
                                    improvement_curve[start_indices_for_combinations[j]]
                                )
                            else:
                                relative_improvement = (abs(improvement_curve[end_index]) - abs(improvement_curve[start_indices_for_combinations[j]])) / abs(improvement_curve[end_index])
                            improvements.append({"Model_combination": combinations[j], "average_relative_improvement": relative_improvement / (end_index - start_indices_for_combinations[j])})
                    if len(improvements) > 0:
                        return max(improvements, key=lambda x: x["average_relative_improvement"])
                else:
                    return {
                        "Model_combination": combinations[0],
                        "average_relative_improvement": ((abs(improvement_curve[start_index]) - abs(improvement_curve[end_index])) / abs(improvement_curve[start_index])),
                    }
