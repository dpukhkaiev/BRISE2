...
class DynamicModelRecommendation(ModelRecommendation):
    def __init__(self, experiment_description: Dict, experiment_id):
        ...
        recommendation_granularity_type = list(self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["RecommendationGranularity"])[0]
        self.recommendation_granularity_value = self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["RecommendationGranularity"][recommendation_granularity_type][
            "Value"
        ]
        self.threshold_type = self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["ThresholdType"]
        self.time_to_build_model_threshold = self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["TimeToBuildModelThreshold"]
        self.time_unit = self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["TimeUnit"]
        ...
        is_minimization = experiment_description["Context"]["TaskConfiguration"]["Objectives"][self.objective_name]["Minimization"]
        self.performance_metric = metric_orchestrator.get_metric(self.experiment_description["TransferLearning"]["ModelRecommendation"][self.feature_name]["PerformanceMetric"], is_minimization)
    ...
    def recommend_best_model(self, similar_experiments: List) -> Union[Dict[Tuple[Hyperparameter], Model], None]:
        ...
        if not self.was_model_recommended:
            current_iteration = self.database.get_last_record_by_experiment_id("Experiment_state", self.experiment_id)["Number_of_measured_configs"]
            ...
            for experiment in similar_experiments:
                ...
                metric = self.performance_metric.compute(improvement_curve, prediction_infos, improvement_start_index, improvement_end_index, multi_model)
                experiments_with_model_metrics.append(
                    {
                        "Experiment": experiment,
                        "metric": metric,
                        "avg_time": self.__get_average_time_to_build_models(metric["Model_combination"], prediction_infos, time_start_index, int(time_end_index)),
                    }
                )
            sorted_experiments_by_metric = sorted(
                filter(lambda experiment_with_metric: experiment_with_metric["metric"]["average_relative_improvement"] is not None, experiments_with_model_metrics),
                key=lambda experiment: experiment["metric"]["average_relative_improvement"],
                reverse=True,
            )
            filtered_experiments_by_time = list(filter(lambda experiment: (experiment["avg_time"] is not None), sorted_experiments_by_metric))
            ...
            while i < len(filtered_experiments_by_time):
                if self.threshold_type == "Soft":
                    ...
                    avg_imp = sum([experiment["metric"]["average_relative_improvement"] for experiment in filtered_experiments_by_time]) / len(filtered_experiments_by_time)
                    if filtered_experiments_by_time[i]["metric"]["average_relative_improvement"] >= avg_imp and filtered_experiments_by_time[i]["metric"]["average_relative_improvement"] > 0:
                        threshold_flag = True
                        if filtered_experiments_by_time[i]["avg_time"] >= self.time_to_build_model_threshold:
                            threshold_flag = False
                        if threshold_flag is True:
                            self.logger.info(f"Recommended combination of models with performance \
                            ...
                            self.logger.info(f"Combination {filtered_experiments_by_time[i]['metric']['Model_combination']} has a good performance \
            ...
            if resulting_best_combination is None:
                ...
                search_space: SearchSpace = pickle.loads(self.database.get_last_record_by_experiment_id("Search_space", self.experiment_id)["SearchspaceObject"])
                ...
                for r_index_str, model_description in resulting_best_combination.items():
                    ...
                    mapping_region_model[search_space.regions[r_index]] = Model(
                        models_types[r_index], search_space.regions[r_index], self.experiment_description["Context"]["TaskConfiguration"]["Objectives"]
                    )
    ...
    @staticmethod
    def __get_average_time_to_build_models(model_combination: dict, prediction_infos: list, start_index: int, end_index: int):
        """
