...
class DefaultConfigHandlerOrchestrator:
    ...
    def get_default_configuration_handler(self, experiment: Experiment) -> DefaultConfigurationHandler:
        if "DefaultConfigurationHandler" in experiment.description.keys():
            ...
            assert len(keys) == 1
            feature_name = keys[0]
            default_configuration_handler_class = reflective_class_import(class_name=default_configuration_handler_description[feature_name]["Type"], folder_path="default_config_handler")
            return default_configuration_handler_class(default_configuration_handler_description[feature_name], experiment)
        else:
