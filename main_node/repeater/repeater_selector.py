...
class RepeaterOrchestration:
    ...
    def __init__(self, experiment_id: str, experiment=None):
        ...
        objectives = [
            self.experiment_description["Context"]["TaskConfiguration"]["Objectives"][key]["Name"] for key in self.experiment_description["Context"]["TaskConfiguration"]["Objectives"].keys()
        ]
        self._objectives = objectives
        data_types = [
            self.experiment_description["Context"]["TaskConfiguration"]["Objectives"][key]["DataType"] for key in self.experiment_description["Context"]["TaskConfiguration"]["Objectives"].keys()
        ]

        self._objectives_data_types = data_types
        minimal_values = [
            self.experiment_description["Context"]["TaskConfiguration"]["Objectives"][key]["MinExpectedValue"]
            for key in self.experiment_description["Context"]["TaskConfiguration"]["Objectives"].keys()
        ]
        maximal_values = [
            self.experiment_description["Context"]["TaskConfiguration"]["Objectives"][key]["MaxExpectedValue"]
            for key in self.experiment_description["Context"]["TaskConfiguration"]["Objectives"].keys()
        ]
    ...
    def measure_configurations(self, channel, method, properties, body):
        ...
        if configuration.status["evaluated"] and os.environ.get("TEST_MODE") != "UNIT_TEST":
            ...
            for index, objective in enumerate(self._objectives):
                tasks_results = error_check(tasks_results, objective, self._expected_values_range[index], self._objectives_data_types[index])
        ...
        if configuration.number_of_failed_tasks <= self.repeater_parameters["MaxFailedTasksPerConfiguration"]:
            ...
            if len(configuration.get_tasks()) == 0:
                publish(exchange="experiment_api_exchange", routing_key=self.experiment_id, body="increment_bad_configuration_number")
                configuration.disable_configuration()
        current_measurement = {str(configuration.parameters): {"parameters": configuration.parameters, "needed_tasks_count": needed_tasks_count, "Finished": False}}
        ...
        for point in current_measurement.keys():
            if not current_measurement[point]["Finished"]:
                for i in range(current_measurement[point]["needed_tasks_count"]):
                    ...
                    self.database.update_record("Experiment_state", {"Exp_unique_ID": self.experiment_id}, {"Number_of_measured_tasks": self.performed_measurements})
        ...
        if os.environ.get("TEST_MODE") == "UNIT_TEST":
            ...
            if configuration.type == Configuration.Type.DEFAULT:
                ...
                publish(exchange="default_configuration_results_exchange", routing_key=self.experiment_id, body=configuration.to_json())
            elif configuration.type == Configuration.Type.PREDICTED or configuration.type == Configuration.Type.FROM_SELECTOR or configuration.type == Configuration.Type.TRANSFERRED:
                publish(exchange="configurations_results_exchange", routing_key=self.experiment_id, body=configuration.to_json())
        elif configuration.status["evaluated"]:
            body = json.dumps({"configuration": configuration.to_json(), "tasks": tasks_to_send})
            publish(exchange="process_tasks_exchange", routing_key=self.experiment_id, body=body)
    ...
    class _EventServiceConnection(RabbitMQConnection):
        ...
        def bind_and_consume(self):
            ...
            self.channel.basic_consume(queue="measurement_results_exchange" + self.experiment_id, auto_ack=True, on_message_callback=self.orchestrator.measure_configurations)
            self.channel.basic_consume(queue="measure_new_configuration_exchange" + self.experiment_id, auto_ack=True, on_message_callback=self.orchestrator.measure_configurations)
