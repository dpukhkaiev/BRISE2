...
class MainThread(threading.Thread):
    ...
    def run(self):
        ...
        if not self.experiment_setup:
            ...
            if len(argv) > 1:
                ...
                log_msg = f"The Experiment Setup was not provided and the path to an experiment file was not specified." f" The default one will be executed: {exp_desc_file_path}"
        ...
        self.connection = pika.BlockingConnection(pika.ConnectionParameters(os.getenv("BRISE_EVENT_SERVICE_HOST"), int(os.getenv("BRISE_EVENT_SERVICE_AMQP_PORT"))))
        self.consume_channel = self.connection.channel()
        self.create_and_bind_queues()
        self.consume_channel.basic_consume(queue="default_configuration_results_exchange" + self.experiment.unique_id, auto_ack=True, on_message_callback=self.get_default_configurations_results)
        self.consume_channel.basic_consume(queue="configurations_results_exchange" + self.experiment.unique_id, auto_ack=True, on_message_callback=self.get_configurations_results)
        self.consume_channel.basic_consume(queue="stop_experiment_exchange" + self.experiment.unique_id, auto_ack=True, on_message_callback=self.stop)
        self.consume_channel.basic_consume(queue="experiment_api_exchange" + self.experiment.unique_id, auto_ack=True, on_message_callback=self.experiment_api)
        self.consume_channel.basic_consume(queue="logging_exchange" + self.experiment.unique_id, auto_ack=True, on_message_callback=self.logging_api)
        self.consume_channel.basic_consume(queue="stop_experiment_queue", auto_ack=True, on_message_callback=self.stop)

        # initialize connection to the database
        self.database = MongoDB(
            os.getenv("BRISE_DATABASE_HOST"), int(os.getenv("BRISE_DATABASE_PORT")), os.getenv("BRISE_DATABASE_NAME"), os.getenv("BRISE_DATABASE_USER"), os.getenv("BRISE_DATABASE_PASS")
        )
        ...
        self.database.write_one_record("Search_space", get_search_space_record(self.experiment.search_space, self.experiment.unique_id))
        self.experiment.send_state_to_db()

        self.sub.send("experiment", "description", global_config={}, experiment_description=self.experiment.description, searchspace_description=self.experiment.search_space.serialize())
        ...
        self.consume_channel.basic_publish(exchange="measure_new_configuration_exchange", routing_key=self.experiment.unique_id, body=body)
    ...
    def get_default_configurations_results(self, ch, method, properties, body):
        ...
        if default_configuration.status["measured"]:
            ...
            self.database.update_record("Search_space", {"Exp_unique_ID": self.experiment.unique_id}, {"Default_configuration": default_configuration.get_configuration_record()})
            self.database.update_record("Search_space", {"Exp_unique_ID": self.experiment.unique_id}, {"SearchspaceObject": pickle.dumps(self.experiment.search_space)})
            ...
            self.consume_channel.basic_publish(exchange="get_worker_capacity_exchange", routing_key=self.experiment.unique_id, body="")
    ...
    def stop(self, ch=None, method=None, properties=None, body=None):
        ...
        with self.conf_lock:
            ...
            self._state = self.State.IDLE
            self.consume_channel.basic_publish(exchange="experiment_termination_exchange", routing_key=self.experiment.unique_id, body="")
    ...
    def create_and_bind_queues(self):
        ...
        """
        self.exchanges = [
            "experiment_termination_exchange",
            "task_result_exchange",
            "measurement_results_exchange",
            "default_configuration_results_exchange",
            "configurations_results_exchange",
            "stop_experiment_exchange",
            "check_stop_condition_expression_exchange",
            "logging_exchange",
            "get_worker_capacity_exchange",
            "get_new_configuration_exchange",
            "measure_new_configuration_exchange",
            "process_tasks_exchange",
            "experiment_api_exchange",
        ]
