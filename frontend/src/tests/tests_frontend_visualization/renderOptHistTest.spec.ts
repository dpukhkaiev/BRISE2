...
describe('renderOptHist', () => {
    ...
    it('purges the plot when allRes is empty', () => {
        ...
        renderOptHist(plotly, element, emptyAllRes.value, experiment_description.value, 'runtime')
    ...
    it('renders the correct labels and values', () => {
        ...
        renderOptHist(plotly, element, testData, experiment_description.value, 'runtime')
    ...
    it('sets the correct layout', () => {
        ...
        renderOptHist(plotly, element, allRes.value, experiment_description.value, 'runtime')
    ...
    it('marks the final solution with a gold star', () => {
        const element = document.createElement('div')

        const solution: PointExp = {
            configurations: {
                frequency: 70
            },
            results: {
        ...
        renderOptHist(plotly, element, allRes.value, experiment_description.value, 'runtime', solution)

        const call = plotly.react.mock.calls[0]
        const data = call[1] as any

        const solutionTrace = data.find((trace: any) => trace.name === 'Solution')

        expect(solutionTrace).toBeDefined()
        expect(solutionTrace.x).toEqual([3])
        expect(solutionTrace.y).toEqual([20])
