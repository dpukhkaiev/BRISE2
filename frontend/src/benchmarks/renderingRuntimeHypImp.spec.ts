...
import { loadBenchmarkPlotly } from "./loadBenchmarkPlotly"
import type { PlotlyInstance } from "../widgets/charts/model/chart.types"
import { renderHypImp } from "../widgets/charts/hyp-imp/render.ts"

import { server } from "vitest/browser"

const { writeFile } = server.commands

describe("Hyperparameter Importances - Rendering Runtime Benchmark", () => {
    let plotly: PlotlyInstance

    beforeAll(async () => {
        plotly = await loadBenchmarkPlotly()
    })
    ...
    it("measures rendering runtime for different numbers of trials", async () => {
        ...
        for (const numTrials of trialCounts) {
            ...
            const result = {importances: {} as Record<string, number>}

            for (const param of Object.keys(experiment.experimentDescription.Context.SearchSpace as Record<string, any>)) {
                result.importances[param] = 1/params
            }
        ...
        await writeFile(
            "./results/hypImp-rendering-trials.csv",
            csv
        )
    }, 120_000)
    ...
    it("measures rendering runtime for different numbers of params", async () => {
        ...
        for (const numParams of paramsCounts) {
            ...
            for (const param of Object.keys(experiment.experimentDescription.Context.SearchSpace as Record<string, any>)) {
                result.importances[param] = 1/numParams
            }
        ...
        await writeFile(
            "./results/hypImp-rendering-params.csv",
            csv
        )
    }, 120_000)
    ...
    it("measures rendering runtime for different numbers of objectives", async () => {
        ...
        await writeFile(
            "./results/hypImp-rendering-objectives.csv",
            csv
        )
    }, 120_000)
})