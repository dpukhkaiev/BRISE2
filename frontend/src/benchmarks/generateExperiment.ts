...
// Only used by the commented-out block in generateObjectiveValue below.
...
function generateObjectiveValue(
    // unused while the block below is commented out; kept (and prefixed) so the signature
    // doesn't change if that block comes back
    _configurations: Record<string, string | number>,
    random: () => number
): number {
    let value = 0

    // Each parameter's value was computed here but never added into `value`, so the
    // objective never actually depended on the configuration - it was pure noise. Left
    // commented out rather than fixed, to keep today's benchmark data unchanged.
