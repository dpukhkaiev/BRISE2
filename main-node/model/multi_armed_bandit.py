...
class MultiArmedBandit(Model):
    ...
    def build_model(
        ...
        if is_minimization:
            raise TypeError(f"Multi Armed Bandit optimizer performs only maximization tasks!")
        self.is_built = False
            ...
            for feature_category in self.features_description[feature_name]['categories']:
                ...
                if isinstance(self.parameters['c'], (int, float)):
                    ...
                    if len(labels[objective_column]) > 1:
                        c = np.std(labels[objective_column])   # * 10  TODO: need to investigate the influence of multiplication
                    else:
