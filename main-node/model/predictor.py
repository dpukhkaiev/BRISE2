    ...
    def predict(self, measured_configurations: List[Configuration]) -> Configuration:
        ...
        while not self.search_space.validate(parameters, is_recursive=True):
            ...
            if model.is_built:
                ...
                if len(prediction) > 1:
                    # maybe, this branch will be used, when we start operate with population and Pareto Front in MO.
                ...
                if not valid_prediction_found:
                    self.logger.warning(f"Model did not predict valid hyperparameter set. Sampling random.")
                    self.search_space.generate(parameters)
                else:
