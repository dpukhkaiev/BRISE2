    ...
    def decision_function(self, history, point, threshold = 15, **configuration):
        ...
        if len(all_experiments) < 2:
            ...
            for index, error in enumerate(relative_errors):
                ...
                threshold = 100/(1+exp(-float((all_dim_avg.tolist()[0][index] / default_point_backup[index]))+3.3)) + len(all_experiments) - 2 if default_point_backup else threshold + len(all_experiments)
