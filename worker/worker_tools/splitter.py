...
class Splitter:
    ...
    def search(self, FR, TR):
        ...
        return (
            {"threads": self.new_data[0]["TR"], "frequency": self.new_data[0]["FR"], "energy": self.new_data[0]["EN"], "time": self.new_data[0]["TIM"]}
            if self.new_data
            else {"worker": "Error! Incorrect worker config"}
        )
    ...
    def searchNB(self, LC, EM, BwS, Bw, MBw, NoK, UAG, AGS):
        ...
        if self.data:
            for i in self.data:
                if (
                    self.__str_to_bool(str(i["laplace_correction"])) == self.__str_to_bool(LC)
                    and i["estimation_mode"] == EM
                    and i["bandwidth_selection"] == BwS
                    and i["bandwidth"] == Bw
                    and i["minimum_bandwidth"] == MBw
                    and i["number_of_kernels"] == NoK
                    and self.__str_to_bool(str(i["use_application_grid"])) == self.__str_to_bool(UAG)
                    and i["application_grid_size"] == AGS
                ):
                    self.new_data.append(i)
        if self.new_data == []:
            self.new_data.append({})
            self.new_data[0]["PREC_AT_99_REC"] = str(0.2435)
            return {"PREC_AT_99_REC": str(0.2435)}
        return {"PREC_AT_99_REC": self.new_data[0]["PREC_AT_99_REC"]} if self.new_data else {"worker": "Error! Incorrect worker config"}
    ...
    def searchGA(self, file_name):
        ...
        try:
            ...
            return (
                ...
                if self.new_data
                else {"worker": "Error! Incorect worker config"}
            )
    ...
    def searchSA(self, file_name):
        ...
        try:
            with open(file_name, "r") as csv_file:
                ...
                self.new_data.append(last_row)
            return (
                {"hardScoreImprovement": self.new_data[0]["hardScoreImprovement"], "softScoreImprovement": self.new_data[0]["softScoreImprovement"]}
                if self.new_data
                else {"worker": "Error! Incorrect worker config"}
            )
        except EnvironmentError:
