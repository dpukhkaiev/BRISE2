...
class JMetalPyWrapper(ILLHWrapper):
    ...
    def construct(self, hyperparameters: Mapping, scenario: Mapping, parameter_control_info: Mapping) -> None:
        ...
        if "crossover_type" in init_args:
            ...
            crossover_class = JMetalPyWrapper._get_class_from_module(name=init_args.pop("crossover_type").split(".")[-1], module=crossover)
        ...
        if "mutation_type" in init_args:
            ...
            mutation_class = JMetalPyWrapper._get_class_from_module(name=init_args.pop("mutation_type").split(".")[-1], module=mutation)
        ...
        if "selection_type" in init_args:
            ...
            if selection_type == "ReuletteWheelSelection":
                ...
                init_args["selection"] = MultiComparator([FastNonDominatedRanking.get_comparator(), CrowdingDistance.get_comparator()])
            else:
                import jmetal.operator.selection as selection

                selection_class = JMetalPyWrapper._get_class_from_module(name=selection_type, module=selection)
        ...
        termination_class = JMetalPyWrapper._get_class_from_module(name=scenario["Budget"]["Type"], module=termination)
        ...
        problem = JMetalPyWrapper._get_problem(problem_name=scenario["Problem"], init_params=problem_init_params)
    ...
    def run_and_report(self) -> Mapping:
        ...
        result = {"objective": current_objective, "improvement": improvement, "parameter_control_info": {"solutions": [s.variables for s in self._llh_algorithm.solutions]}}
        return result
    ...
    def load_initial_solutions(self, parameter_control_info: Mapping, problem: Problem) -> None:
        ...
        if parameter_control_info and "solutions" in self.parameter_control_info.keys() and len(self.parameter_control_info["solutions"]) > 0:
            from jmetal.util.generator import InjectorGenerator

            one_sol_variables = self.parameter_control_info["solutions"][0]
            if self.problem_type == "permutation_problem" or self.problem_type is None:
                solution = PermutationSolution(number_of_variables=problem.number_of_variables, number_of_objectives=problem.number_of_objectives)
            elif self.problem_type == "float_problem":
                solution = FloatSolution(lower_bound=problem.lower_bound, upper_bound=problem.upper_bound, number_of_objectives=problem.number_of_objectives)
            elif self.problem_type == "binary_problem":
                solution = BinarySolution(number_of_variables=problem.number_of_variables, number_of_objectives=problem.number_of_objectives)
    ...
    def construct(self, hyperparameters: Mapping, scenario: Mapping, parameter_control_info: Mapping) -> None:
        ...
        if mh_name == "GeneticAlgorithm":
            ...
            self._llh_algorithm = SimulatedAnnealing(
                problem=problem, mutation=PermutationSwapMutation(0.8954537798654278), termination_criterion=termination_criterion, solution_generator=self._solution_generator
            )
...
class JMetalPyWrapperDefault(JMetalPyWrapper):
    def construct(self, hyperparameters: Mapping, scenario: Mapping, parameter_control_info: Mapping) -> None:
        ...
        if mh_name == "GeneticAlgorithm":
            ...
            self._llh_algorithm = SimulatedAnnealing(problem=problem, mutation=PermutationSwapMutation(0.5), termination_criterion=termination_criterion, solution_generator=self._solution_generator)

        elif mh_name == "EvolutionStrategy":
            from jmetal.algorithm.singleobjective.evolution_strategy import EvolutionStrategy

            self._llh_algorithm = EvolutionStrategy(
                problem=problem, mu=500, lambda_=500, elitist=False, mutation=PermutationSwapMutation(0.5), termination_criterion=termination_criterion, population_generator=self._solution_generator
            )
        else:
            self.logger.error(f"Wrong meta-heuristic name: {mh_name}")
